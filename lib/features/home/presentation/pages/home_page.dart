...
class HomeAssetPaths {
  ...
  static const deviceCardSharingBadge =
      'assets/icons/home/home_device_card_sharing_badge.png';
}
              ...
              children: [
                ...
                if (isSingleColumn)
                  for (final device in home.devices) ...[
                    SizedBox(height: 162, child: _DeviceCard(device: device)),
      ...
      child: Material(
        ...
        child: Stack(
          children: [
            Positioned(
              top: isSharing
                  ? _sharingBadgeSize + _statusDotSpacing
                  : _cardInset,
              right: _cardInset,
              child: _DeviceStatusDot(
                key: _statusDotKey,
...
class _DeviceEditingSheetState extends ConsumerState<_DeviceEditingSheet> {
  ...
  Future<void> _shareDevice() async {
    final noDeviceMessage = AppLocalizations.of(
      widget.parentContext,
    ).homeNoDeviceMessage;
    ...
    if (widget.parentContext.mounted) {
      GoRouter.of(
        widget.parentContext,
      ).push(DeviceSharePage.routePath, extra: widget.device);
    }
  }
...
class _DeviceDoorIcon extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (coverImage != null) {
      return Image.network(
        coverImage.url,
        key: ValueKey<String>('home-device-cover-${device.id}'),
        ...
        errorBuilder: (context, error, stackTrace) =>
            _DefaultDeviceDoorIcon(visual: visual),
      );
    }

    return _DefaultDeviceDoorIcon(visual: visual);
  }
}

class _DefaultDeviceDoorIcon extends StatelessWidget {
  const _DefaultDeviceDoorIcon({required this.visual});
