...
class _DeviceCommandPageState extends ConsumerState<DeviceCommandPage> {
  static const _garageDoorClosedAsset = 'assets/images/device_control_garage_door_closed.png';
  static const _garageDoorFallbackAsset = 'assets/icons/add_device/add_new_doors_garage_door.png';
  ...
  Widget build(BuildContext context) {
    ...
    return IndexedStack(
      ...
      children: [
        ...
        SecurityCenterPage(deviceId: _hardwareDeviceId(commandState), onTabSelected: _selectTab),
      ],
    );
  }
  ...
  String _hardwareDeviceId(DeviceCommandState commandState) {
    if (commandState.bleConnectionStatus == DeviceBleConnectionStatus.connected && (commandState.bleDeviceId?.trim().isNotEmpty ?? false)) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: ListView(
          ...
          children: [
            _CycleSummary(operatedCycles: doorDetail?.operatedCycles, remainingCycles: doorDetail?.remainingCycles, textTheme: textTheme),
            const SizedBox(height: 5),
            const _DeviceConnectionStrip(),
            const SizedBox(height: 12),
            _DoorHeroImage(assetPath: _garageDoorClosedAsset, fallbackAssetPath: _garageDoorFallbackAsset),
            const SizedBox(height: 4),
            Center(child: Text(doorDetail?.doorStateLabel ?? 'Closed', style: AppTextTokens.deviceControlDoorState(textTheme))),
            ...
            _QuickActionGrid(
              ...
              onLedChanged: (enabled) async {
                ...
                await controller.runAction(deviceId: hardwareDeviceId, action: enabled ? DeviceCommandAction.turnLightOn : DeviceCommandAction.turnLightOff);
              },
...
abstract final class _DeviceCommandAssetPaths {
  static const video = 'assets/icons/device_control/device_command_video_placeholder.png';
  static const motor = 'assets/icons/device_control/device_command_motor_placeholder.png';
  static const remote = 'assets/icons/device_control/device_command_remote_placeholder.png';
  static const sensor = 'assets/icons/device_control/device_command_sensor_placeholder.png';
  static const solar = 'assets/icons/device_control/device_command_solar_placeholder.png';
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: IconButton(
        ...
        onPressed: () {},
        icon: const _DeviceControlAssetIcon(assetPath: _DeviceCommandAssetPaths.video),
      ),
    );
  }
}
    ...
    return const DecoratedBox(
      ...
      child: Padding(
        ...
        child: FittedBox(
          ...
          child: Row(
            children: [
              _ConnectionGroup(deviceIconAsset: _DeviceCommandAssetPaths.motor, bluetoothActive: true, wifiActive: true),
              SizedBox(width: 22),
              _ConnectionGroup(deviceIconAsset: _DeviceCommandAssetPaths.remote, bluetoothActive: false, wifiActive: true),
              SizedBox(width: 22),
              _ConnectionGroup(deviceIconAsset: _DeviceCommandAssetPaths.sensor, bluetoothActive: false, wifiActive: false),
              SizedBox(width: 22),
              _ConnectionGroup(deviceIconAsset: _DeviceCommandAssetPaths.solar, bluetoothActive: false, wifiActive: true),
            ],
          ),
        ),
      ),
...
class _ConnectionGroup extends StatelessWidget {
  const _ConnectionGroup({required this.deviceIconAsset, required this.bluetoothActive, required this.wifiActive});
...
class _DoorHeroImage extends StatelessWidget {
  const _DoorHeroImage({required this.assetPath, required this.fallbackAssetPath});
        ...
        child: Image.asset(
          ...
          errorBuilder: (context, error, stackTrace) {
            return DecoratedBox(
              ...
              child: Center(child: Image.asset(fallbackAssetPath, width: 180, fit: BoxFit.contain)),
            );
          },
        ),
      ),
...
class _DoorCommandRow extends StatelessWidget {
  const _DoorCommandRow({required this.busy, required this.pendingAction, required this.onClose, required this.onStop, required this.onOpen});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        _DoorCommandButton(tooltip: 'Stop', icon: Icons.pause, pending: pendingAction == DeviceCommandAction.stopDoor, onPressed: busy ? null : onStop),
        const SizedBox(width: 34),
        _DoorCommandButton(
          tooltip: 'Open',
        ...
        children: [
          Expanded(
            child: Column(
              children: [
                Expanded(
                  child: _LedActionCard(enabled: ledEnabled, busy: busy, textTheme: textTheme, onChanged: onLedChanged),
                ),
          ...
          Expanded(
            child: Column(
              children: [
                Expanded(
                  flex: 140,
                  child: _PartialOpenCard(busy: busy, textTheme: textTheme, onPressed: onPartialOpen),
                ),
                const SizedBox(height: 12),
                Expanded(
                  flex: 44,
...
class _LedActionCard extends StatelessWidget {
  const _LedActionCard({required this.enabled, required this.busy, required this.textTheme, required this.onChanged});
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              FlinxSwitch(key: const ValueKey<String>('led-switch'), value: enabled, enabled: !busy, onChanged: onChanged),
            ],
          ),
          const Spacer(),
          Padding(
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              FlinxSwitch(key: switchKey, value: enabled, enabled: !busy, onChanged: onChanged),
            ],
          ),
          const Spacer(),
...
class _PartialOpenCard extends StatelessWidget {
  const _PartialOpenCard({required this.busy, required this.textTheme, required this.onPressed});
          ...
          Align(
            ...
            child: DecoratedBox(
              ...
              child: Padding(
                ...
                child: Text('60cm', style: AppTextTokens.deviceControlBadge(textTheme)),
              ),
            ),
          ),
          Center(
