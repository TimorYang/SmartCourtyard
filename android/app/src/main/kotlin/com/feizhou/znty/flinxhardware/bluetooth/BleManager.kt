package com.feizhou.znty.flinxhardware.bluetooth
...
import com.feizhou.znty.flinxhardware.bridge.BleDiagnosticDirectionDto
import com.feizhou.znty.flinxhardware.bridge.BleDiagnosticEventDto
import com.feizhou.znty.flinxhardware.bridge.BleCharacteristicDto
import com.feizhou.znty.flinxhardware.bridge.BleAuthenticationResultDto
import com.feizhou.znty.flinxhardware.bridge.BleConnectionEventDto
import com.feizhou.znty.flinxhardware.bridge.BleConnectionStateDto
import com.feizhou.znty.flinxhardware.bridge.BleDeviceDto
import com.feizhou.znty.flinxhardware.bridge.BleNotificationDto
import com.feizhou.znty.flinxhardware.bridge.BleReadResultDto
import com.feizhou.znty.flinxhardware.bridge.BleScanFilterDto
import com.feizhou.znty.flinxhardware.bridge.BleServiceDto
import com.feizhou.znty.flinxhardware.bridge.BleServicesDto
import com.feizhou.znty.flinxhardware.bridge.BleWriteResultDto
import com.feizhou.znty.flinxhardware.bridge.BleWriteTypeDto
import com.feizhou.znty.flinxhardware.bridge.CommandResultDto
import com.feizhou.znty.flinxhardware.bridge.ConnectedBleDeviceDto
import com.feizhou.znty.flinxhardware.bridge.FlutterError
import com.feizhou.znty.flinxhardware.bridge.WifiProvisionResultDto
import com.feizhou.znty.flinxhardware.bridge.WifiScanResultDto
import com.feizhou.znty.flinxhardware.protocol.DeviceBleAesKeyCandidate
import com.feizhou.znty.flinxhardware.protocol.DeviceBleFrame
import com.feizhou.znty.flinxhardware.protocol.DeviceBleDecodeStatus
...
class BleManager(
  ...
  companion object {
    ...
    private const val PROVISIONING_COMMAND_TIMEOUT_MS = 15_000L
  ...
  /** 扫描设备附近 Wi-Fi。请求按 BLE 协议加密发送，设备回包按明文帧解析。 */
  @SuppressLint("MissingPermission")
  fun scanWifiNetworks(
    ...
    val gatt = gattMap[deviceId]
      ?: throw FlutterError("bluetooth_disconnected", "BLE device is not connected.")
    if (wifiScanSessions.containsKey(deviceId)) {
      callback(
        Result.failure(
          FlutterError(
            "operation_in_progress",
            "BLE Wi-Fi scan is already in progress.",
            "deviceId=$deviceId",
          ),
        ),
      )
    ...
    val timeoutRunnable = Runnable {
      val removed = wifiScanSessions.remove(deviceId) ?: return@Runnable
      removed.callback(
        Result.failure(
          FlutterError(
            "command_timeout",
            "BLE Wi-Fi scan timed out.",
    ...
    if (!gatt.writeCharacteristic(writeCharacteristic)) {
      removeWifiScanSession(deviceId)
      callback(
        Result.failure(
          FlutterError(
            "write_characteristic_failed",
            "Failed to send BLE Wi-Fi scan frame.",
            "deviceId=$deviceId",
          ),
        ),
      )
  ...
  private fun removeWifiScanSession(deviceId: String): PendingWifiScan? {
    val pending = wifiScanSessions.remove(deviceId) ?: return null
    pending.timeoutRunnable?.let(mainHandler::removeCallbacks)
    return pending
  }
  ...
  private fun handleWifiScanNotification(
    ...
    val pending = wifiScanSessions[deviceId] ?: return
    ...
    removeWifiScanSession(deviceId)
    val ssids = runCatching { parseWifiSsids(frame.data) }.getOrElse { error ->
      pending.callback(
        Result.failure(
          FlutterError(
    ...
    Log.d(
      TAG,
      "WiFi扫描结果 requestId=${pending.requestId} callbackRequestId=${requestId ?: "unknown"} deviceId=$deviceId seq=0x${frame.sequence.toString(16).padStart(4, '0')} count=${ssids.size} ssids=$ssids",
    )
    pending.callback(
      Result.success(
        WifiScanResultDto(
  ...
  private fun parseWifiSsids(data: ByteArray): List<String> {
    val jsonBytes = if (
      data.size > 1 &&
      (data[0] == 0x00.toByte() || data[0] == 0x01.toByte()) &&
      data[1] == '['.code.toByte()
    ) {
    ...
    val json = String(jsonBytes, StandardCharsets.UTF_8).trim()
    val array = JSONArray(json)
    val ssids = mutableListOf<String>()
    for (index in 0 until array.length()) {
      val ssid = array.optString(index).trim()
      if (ssid.isNotEmpty()) {
        ssids += ssid
      }
    }
